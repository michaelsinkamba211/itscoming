  ...
  return (
    <li>
      ...
      >
        {children}
        {isActive && (
          <span className="absolute inset-x-1 -bottom-px h-px bg-gradient-to-r from-green-800/0 via-green-800/40 to-green-800/0" />
        )}
      </Link>
    </li>
  )
...
function AvatarContainer({ className, ...props }) {
  return (
    <div
      className={clsx(
        className,
        'h-11 w-auto rounded-full bg-white/90 p-1 shadow-lg shadow-zinc-800/5 ring-1 ring-zinc-900/5 backdrop-blur'
      )}
      {...props}
    />
  )
...
export function Header() {
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('scroll', updateHeaderStyles, { passive: true })
